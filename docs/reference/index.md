# Package index

## All functions

- [`elcf4R`](https://fbertran.github.io/elcf4R/reference/elcf4R-package.md)
  [`elcf4R-package`](https://fbertran.github.io/elcf4R/reference/elcf4R-package.md)
  : Forecasting Individual Electricity Load Curves

- [`elcf4r_assign_kwf_clusters()`](https://fbertran.github.io/elcf4R/reference/elcf4r_assign_kwf_clusters.md)
  : Assign segments to a fitted KWF clustering model

- [`elcf4r_benchmark()`](https://fbertran.github.io/elcf4R/reference/elcf4r_benchmark.md)
  : Run a rolling-origin benchmark on a normalized panel

- [`elcf4r_build_benchmark_index()`](https://fbertran.github.io/elcf4R/reference/elcf4r_build_benchmark_index.md)
  : Build a day-level benchmark index from a normalized panel

- [`elcf4r_build_daily_segments()`](https://fbertran.github.io/elcf4R/reference/elcf4r_build_daily_segments.md)
  : Build daily load-curve segments from a normalized panel

- [`elcf4r_calendar_groups()`](https://fbertran.github.io/elcf4R/reference/elcf4r_calendar_groups.md)
  : Derive deterministic KWF calendar groups

- [`elcf4r_classify_thermosensitivity()`](https://fbertran.github.io/elcf4R/reference/elcf4r_classify_thermosensitivity.md)
  : Classify thermosensitivity from daily load data

- [`elcf4r_download_elmas()`](https://fbertran.github.io/elcf4R/reference/elcf4r_download_elmas.md)
  : Download the ELMAS dataset from figshare

- [`elcf4r_download_gx()`](https://fbertran.github.io/elcf4R/reference/elcf4r_download_gx.md)
  : Download selected GX dataset components

- [`elcf4r_download_ideal()`](https://fbertran.github.io/elcf4R/reference/elcf4r_download_ideal.md)
  : Download selected IDEAL dataset components

- [`elcf4r_download_storenet()`](https://fbertran.github.io/elcf4R/reference/elcf4r_download_storenet.md)
  : Download one or more StoreNet household files from figshare

- [`elcf4r_elmas_toy`](https://fbertran.github.io/elcf4R/reference/elcf4r_elmas_toy.md)
  : Toy subset of ELMAS hourly cluster profiles

- [`elcf4r_fit_gam()`](https://fbertran.github.io/elcf4R/reference/elcf4r_fit_gam.md)
  : Fit a GAM model for load curves

- [`elcf4r_fit_kwf()`](https://fbertran.github.io/elcf4R/reference/elcf4r_fit_kwf.md)
  : Fit a Kernel Wavelet Functional model for daily load curves

- [`elcf4r_fit_kwf_clustered()`](https://fbertran.github.io/elcf4R/reference/elcf4r_fit_kwf_clustered.md)
  : Fit a clustered KWF model for daily load curves

- [`elcf4r_fit_lstm()`](https://fbertran.github.io/elcf4R/reference/elcf4r_fit_lstm.md)
  : Fit an LSTM model for daily load curves

- [`elcf4r_fit_mars()`](https://fbertran.github.io/elcf4R/reference/elcf4r_fit_mars.md)
  : Fit a MARS model for load curves

- [`elcf4r_iflex_benchmark_index`](https://fbertran.github.io/elcf4R/reference/elcf4r_iflex_benchmark_index.md)
  : iFlex benchmark index of complete participant-days

- [`elcf4r_iflex_benchmark_results`](https://fbertran.github.io/elcf4R/reference/elcf4r_iflex_benchmark_results.md)
  : iFlex benchmark results for shipped forecasting methods

- [`elcf4r_iflex_example`](https://fbertran.github.io/elcf4R/reference/elcf4r_iflex_example.md)
  : iFlex example panel for package examples

- [`elcf4r_kwf_cluster_days()`](https://fbertran.github.io/elcf4R/reference/elcf4r_kwf_cluster_days.md)
  : Cluster daily segments for clustered KWF

- [`elcf4r_lcl_benchmark_results`](https://fbertran.github.io/elcf4R/reference/elcf4r_lcl_benchmark_results.md)
  : Low Carbon London benchmark results for shipped forecasting methods

- [`elcf4r_lcl_example`](https://fbertran.github.io/elcf4R/reference/elcf4r_lcl_example.md)
  : Low Carbon London example panel for package examples

- [`elcf4r_metrics()`](https://fbertran.github.io/elcf4R/reference/elcf4r_metrics.md)
  : Forecast accuracy metrics for load curves

- [`elcf4r_normalize_panel()`](https://fbertran.github.io/elcf4R/reference/elcf4r_normalize_panel.md)
  : Normalize a load panel to the elcf4R schema

- [`elcf4r_read_gx()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_gx.md)
  : Read and normalize the GX residential transformer-level scaffold

- [`elcf4r_read_ideal()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_ideal.md)
  : Read and normalize the IDEAL hourly aggregate-electricity scaffold

- [`elcf4r_read_iflex()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_iflex.md)
  : Read and normalize the iFlex hourly dataset

- [`elcf4r_read_lcl()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_lcl.md)
  : Read and normalize the Low Carbon London dataset

- [`elcf4r_read_refit()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_refit.md)
  : Read and normalize the REFIT cleaned household dataset

- [`elcf4r_read_storenet()`](https://fbertran.github.io/elcf4R/reference/elcf4r_read_storenet.md)
  : Read and normalize the StoreNet household dataset

- [`elcf4r_refit_benchmark_results`](https://fbertran.github.io/elcf4R/reference/elcf4r_refit_benchmark_results.md)
  : REFIT benchmark results for shipped forecasting methods

- [`elcf4r_refit_example`](https://fbertran.github.io/elcf4R/reference/elcf4r_refit_example.md)
  : REFIT example panel for package examples

- [`elcf4r_storenet_benchmark_results`](https://fbertran.github.io/elcf4R/reference/elcf4r_storenet_benchmark_results.md)
  : StoreNet benchmark results for shipped forecasting methods

- [`elcf4r_storenet_example`](https://fbertran.github.io/elcf4R/reference/elcf4r_storenet_example.md)
  : StoreNet example panel for package examples

- [`elcf4r_use_tensorflow_env()`](https://fbertran.github.io/elcf4R/reference/elcf4r_use_tensorflow_env.md)
  : Select the Python environment used for TensorFlow-backed LSTM fits

- [`predict(`*`<elcf4r_kwf_clusters>`*`)`](https://fbertran.github.io/elcf4R/reference/predict.elcf4r_kwf_clusters.md)
  : Assign new segments to a fitted KWF clustering model

- [`predict(`*`<elcf4r_model>`*`)`](https://fbertran.github.io/elcf4R/reference/predict.elcf4r_model.md)
  :

  Predict from an `elcf4r_model`
