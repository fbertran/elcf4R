...
- `IDEAL`: candidate future adapter/helper. The current source record indicates
  `CC BY 4.0`, so the previous non-commercial wording should not be reused.
- `GX`: candidate secondary benchmark dataset. It is transformer-level rather
  than individual-household data, and its licence should be re-verified at the
  dataset-record level before any shipped subset is considered.
